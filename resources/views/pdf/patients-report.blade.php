...
<body>
    <div class="header">
        <div class="header-table">
            ...
            <div class="header-center">
                <h1>{{ $tenant->name ?? 'Relatório de Pacientes' }}</h1>
                <div class="subtitle">{{ $patients->count() }} {{ $patients->count() === 1 ? 'paciente' : 'pacientes' }} &mdash; Gerado em {{ now()->setTimezone('America/Sao_Paulo')->format('d/m/Y \à\s H:i') }}</div>
            </div>
    ...
    <h2 style="text-align: center; font-size: 13px; color: #2d3748; margin-bottom: 15px; letter-spacing: 0.5px;">Relatório de Pacientes</h2>

    @if($patients->isEmpty())
        <div style="text-align: center; padding: 40px 20px; color: #718096; font-size: 12px;">
            Não existem pacientes registrados.
        </div>
    @else
        <table>
            <thead>
                ...
                @foreach($patients as $patient)
                    <tr>
                        ...
                        @foreach($questions as $question)
                            <td>
                                @php
                                    $answer = $patient->answers->firstWhere('question_id', $question->id);
                                @endphp
                                {{ $answer?->answer ?? '-' }}
                            </td>
                        @endforeach
                        <td class="col-date">{{ \Carbon\Carbon::parse($patient->created_at)->setTimezone('America/Sao_Paulo')->format('d/m/Y H:i') }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
