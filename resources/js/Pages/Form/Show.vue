...
// ✅ CORREÇÃO: Usar props.form ao invés de form direto
...
<template>
    ...
    <CentralAdminLayout>
        ...
        <nav class="flex items-center gap-2 text-sm text-gray-500 mb-4" aria-label="Breadcrumb">
            <Link :href="route('dashboard')" class="hover:text-cyan-600 flex items-center gap-1 transition-colors">
                <Home class="w-4 h-4" aria-hidden="true" />
                <span>Início</span>
            </Link>
            <ChevronRight class="w-4 h-4" aria-hidden="true" />
            <Link :href="route('forms.index')" class="hover:text-cyan-600 transition-colors">
                Formulários
            </Link>
            <ChevronRight class="w-4 h-4" aria-hidden="true" />
