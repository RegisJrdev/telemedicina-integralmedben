...
const nameValid = computed(() => {
    if (!form.name || !nameTouched.value) return null;
...
const passwordsMatch = computed(() => {
    if (!form.password_confirmation || !passwordConfirmationTouched.value) return null;
...
const submit = () => {
    ...
    form.post(route('register'), {
        ...
        onError: (errors) => {
            ...
            showToast(errorMessages[0] || 'Erro ao criar conta. Tente novamente.', 'error');
        },
        onFinish: () => {
            form.reset('password', 'password_confirmation');
            passwordTouched.value = false;
            passwordConfirmationTouched.value = false;
        },
    });
};
</script>
                ...
                <form @submit.prevent="submit" class="mt-8 space-y-5">
                    ...
                    <div class="relative">
                        ...
                        <div class="relative">
                            ...
                            <TextInput id="name" type="text" v-model="form.name" @blur="nameTouched = true" required
                                autofocus autocomplete="name" placeholder="Seu nome completo"
                                ...
                                :class="{
                                    'border-destructive focus:border-destructive focus:ring-destructive/20': nameValid === false,
                                    'border-green-300 focus:border-green-500 focus:ring-green-500/20': nameValid === true
                                }" />
                            <!-- Ícone de Validação -->
                            <div v-if="nameValid !== null"
                                class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <svg v-if="nameValid" class="h-5 w-5 text-green-500" fill="currentColor"
                                    viewBox="0 0 20 20">
                        ...
                        <p v-if="nameValid === false" class="mt-1 text-xs text-destructive flex items-center">
                    ...
                    <div class="relative">
                        ...
                        <div class="relative">
                            ...
                            <TextInput id="password_confirmation" :type="showPasswordConfirmation ? 'text' : 'password'"
                                v-model="form.password_confirmation" @blur="passwordConfirmationTouched = true" required
                                autocomplete="new-password" placeholder="••••••••"
                                class="block w-full pl-10 pr-12 py-3 border-2 border-input rounded-xl text-foreground placeholder-muted-foreground focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 ease-in-out hover:border-border"
                                :class="{
                                    'border-destructive focus:border-destructive focus:ring-destructive/20': passwordsMatch === false,
                                    'border-green-300 focus:border-green-500 focus:ring-green-500/20': passwordsMatch === true
                                }" />
                        ...
                        <!-- Ícone de Validação das Senhas -->
                        <div v-if="passwordsMatch !== null" class="mt-1 flex items-center text-xs"
                            :class="passwordsMatch ? 'text-green-600' : 'text-destructive'">
                            <svg v-if="passwordsMatch" class="h-4 w-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                                    d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                    clip-rule="evenodd" />
                            </svg>
